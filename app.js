...
const MODEL_PRESETS = {
  ...
  "runninghub-ai-removebg": {
    matting: "runninghub-ai-app",
    toleranceScale: 1,
  },
};
...
const RUNNINGHUB_AI_APP_REMOVE_BG_PROXY_URL = APP_CONFIG.RUNNINGHUB_AI_APP_REMOVE_BG_PROXY_URL || "";
const RUNNINGHUB_AI_APP_REMOVE_BG_APP_ID = APP_CONFIG.RUNNINGHUB_AI_APP_REMOVE_BG_APP_ID || "1950866462321876993";
const RUNNINGHUB_AI_APP_REMOVE_BG_LABEL = APP_CONFIG.RUNNINGHUB_AI_APP_REMOVE_BG_LABEL || "RunningHub AI 抠图";
const REMOVE_BG_AI_APPS = normalizeRemoveBgAiApps(APP_CONFIG.REMOVE_BG_AI_APPS);
  ...
  for (let poll = 1; poll <= VIDEO_MAX_POLLS; poll += 1) {
    ...
    if (statusResponse.ok && /^video\//i.test(contentType)) {
      ...
      return {
        ...
        resultKind: statusResponse.headers.get("X-Video-Result-Kind") || "",
        requiresLocalChromaKey: statusResponse.headers.get("X-Video-Requires-Local-Chroma-Key") !== "false",
      };
    }
    ...
    if (data?.status === "done" && data.resultUrl) {
      ...
      return {
        blob: resultBlob,
        name: data.filename || outputName,
        mimeType: data.mimeType || resultBlob.type || "",
        resultKind: data.resultKind || "",
        requiresLocalChromaKey: data.requiresLocalChromaKey !== false,
      };
    }
  }
...
function normalizeRemoveBgAiApps(apps) {
  const configuredApps = Array.isArray(apps) ? apps : [];
  const defaults = [
    {
      id: "runninghub-ai-removebg",
      label: RUNNINGHUB_AI_APP_REMOVE_BG_LABEL,
      provider: "runninghub-ai-app",
      appId: RUNNINGHUB_AI_APP_REMOVE_BG_APP_ID,
      default: false,
    },
  ];
  const seen = new Set([...BUILT_IN_REMOVE_BG_MODELS.map((model) => model.id), ...REMOVE_BG_WORKFLOWS.map((workflow) => workflow.id)]);
  return [...configuredApps, ...defaults]
    .map((app) => {
      const id = String(app?.id || "").trim();
      const label = String(app?.label || "").trim();
      const provider = String(app?.provider || "runninghub-ai-app").trim();
      const appId = String(app?.appId || app?.app_id || RUNNINGHUB_AI_APP_REMOVE_BG_APP_ID || "").trim();
      if (!id || !label || provider !== "runninghub-ai-app" || !appId || seen.has(id)) return null;
      seen.add(id);
      return {
        id,
        label,
...
function getRemoveBgChoices() {
  const local = getRemoveBgChoiceFromList("local-fast", BUILT_IN_REMOVE_BG_MODELS);
...
function getPositiveConfigNumber(key, fallback) {
  const value = Number(APP_CONFIG?.[key]);
...
function shouldUseRemoteMatting(options) {
  return ["pixian", "koukoutu", "runninghub", "runninghub-ai-app"].includes(options.removeBgProvider) && options.mode !== "upscale";
}
...
async function removeBackgroundWithProvider(file, canvas, options, item = null) {
  ...
  if (options.removeBgProvider === "runninghub") return removeBackgroundWithRunningHub(file, options, item, canvas);
  if (options.removeBgProvider === "runninghub-ai-app") return removeBackgroundWithRunningHubAiApp(file, options, item, canvas);
...
async function removeBackgroundWithRunningHub(file, options, item = null, sourceCanvas = null) {
  ...
  const originalCanvas = sourceCanvas || canvasFromBitmap(item?.bitmap);
  ...
  console.log(`[${label}] request`, {
    ...
    fileSize: file.size,
    fileType: file.type || "unknown",
    inputWidth: originalCanvas.width,
...
async function removeBackgroundWithRunningHubAiApp(file, options, item = null, sourceCanvas = null) {
  const label = getRemoveBgStatusLabel(options);
  if (!RUNNINGHUB_AI_APP_REMOVE_BG_PROXY_URL) {
    throw createRunningHubProviderError("RunningHub AI 抠图服务未配置。", {
      stage: "config",
      detail: "请先部署 runninghub-ai-removebg-worker.js，并把 Worker 地址填入 config.js 的 RUNNINGHUB_AI_APP_REMOVE_BG_PROXY_URL。",
    });
  }
  ...
  if (item?.cancelRequested || signal?.aborted) throw createCanceledTaskError();
  setStatus("正在准备 RunningHub AI 抠图图像...");
  const uploadInput = await prepareRunningHubRemoveBgInput(file, originalCanvas);
  if (item?.cancelRequested || signal?.aborted) throw createCanceledTaskError();
  if (uploadInput.wasResized) {
    setStatus(`已生成云端输入图：${originalCanvas.width}x${originalCanvas.height} → ${uploadInput.width}x${uploadInput.height}`);
  ...
  const outputResult = await requestRunningHubRemoveBg(uploadInput.blob, {
    label,
    workflowId: options.removeBgChoice?.appId || RUNNINGHUB_AI_APP_REMOVE_BG_APP_ID,
    proxyUrl: RUNNINGHUB_AI_APP_REMOVE_BG_PROXY_URL,
    uploadName: uploadInput.name,
    signal,
    onPoll: (pollCount, message) => {
      if (!item || item.cancelRequested) return;
      const taskSuffix = item.runninghubTaskId ? ` · taskId: ${item.runninghubTaskId}` : "";
      const suffix = pollCount > 0 ? `，第 ${pollCount} 次检查${taskSuffix}` : taskSuffix;
      setCardStatus(item, `${message}${suffix}`, "is-working");
    },
    ...
    createMessage: "RunningHub AI 抠图任务已创建",
    runningMessage: "RunningHub AI 抠图处理中...",
    timeoutMessage: "RunningHub AI 抠图长时间未返回结果，请稍后重试或换小图。",
  });

  if (outputResult.resultType === "mask") setStatus("RunningHub AI 抠图已返回 mask，正在合成透明 PNG...");
  else if (outputResult.resultType === "transparent") setStatus("RunningHub AI 抠图已返回透明结果，正在下载...");
  else setStatus("RunningHub AI 抠图未返回 mask，已使用可下载结果图。");
...
async function requestRunningHubRemoveBg(
  file,
  {
    label,
    workflowId,
    proxyUrl = REMOVE_BG_PROXY_URL,
    uploadName = "input.png",
    signal = null,
    onPoll = null,
    onStatus = null,
    onTaskCreated = null,
    createMessage = "RMBG-2.0 高质量抠图任务已创建",
    runningMessage = "RunningHub 运行中...",
    timeoutMessage = "RunningHub 长时间未返回结果，请稍后重试或换小图。RunningHub 可能仍在后台运行",
  },
) {
  ...
  onStatus?.(task.message || createMessage);
  onPoll?.(0, task.message || createMessage);
  ...
  for (let pollCount = 1; pollCount <= RUNNINGHUB_REMOVEBG_MAX_POLLS; pollCount += 1) {
    ...
    const statusResponse = await requestRunningHubRemoveBgStatus(task.taskId, signal, proxyUrl);
    ...
    onPoll?.(pollCount, runningMessage);
  }

  throw createRunningHubProviderError(timeoutMessage, {
    stage: "poll_task",
...
async function prepareRunningHubRemoveBgInput(file, sourceCanvas) {
  const sourceLongestSide = Math.max(sourceCanvas.width, sourceCanvas.height);
  const needsResize = file.size > RUNNINGHUB_REMOVEBG_MAX_UPLOAD_BYTES_HINT || sourceLongestSide > RUNNINGHUB_REMOVEBG_MAX_SOURCE_SIDE;
  const uploadScale = needsResize ? Math.min(1, RUNNINGHUB_REMOVEBG_MAX_UPLOAD_SIDE / sourceLongestSide) : 1;
  const uploadCanvas =
    uploadScale < 1
      ? resizeCanvasTo(sourceCanvas, Math.round(sourceCanvas.width * uploadScale), Math.round(sourceCanvas.height * uploadScale))
      : sourceCanvas;
  ...
  if (shouldApplyMask) {
    ...
    if (looksLikeScaledFullFrame) {
      ...
      if (placed.referenceBBox) {
        ...
        console.warn(`[${label}] removebg cropped result could not estimate original subject bbox`, {
          originalSize: `${originalWidth}x${originalHeight}`,
...
function isSimilarAspectRatio(widthA, heightA, widthB, heightB, limit = 0.035) {
  const ratioA = widthA / Math.max(1, heightA);
  const ratioB = widthB / Math.max(1, heightB);
...
function isCroppedTransparentResult(resultCanvas, originalCanvas, alphaBBox = getAlphaBoundingBox(resultCanvas)) {
  ...
  const edgeMargin = Math.max(3, Math.round(Math.min(resultCanvas.width, resultCanvas.height) * 0.018));
  const touchesEdge =
    alphaBBox.x <= edgeMargin ||
    alphaBBox.y <= edgeMargin ||
    alphaBBox.x + alphaBBox.width >= resultCanvas.width - edgeMargin ||
    alphaBBox.y + alphaBBox.height >= resultCanvas.height - edgeMargin;
  const bboxAreaRatio = (alphaBBox.width * alphaBBox.height) / Math.max(1, resultCanvas.width * resultCanvas.height);
  const subjectAlmostFillsCanvas = bboxAreaRatio >= 0.82;

  return ratioMismatch || (touchesEdge && bboxAreaRatio >= 0.48) || subjectAlmostFillsCanvas;
}

function getAlphaBoundingBox(canvas, threshold = 8) {
  const ctx = canvas.getContext("2d", { willReadFrequently: true });
...
function placeRunningHubCroppedResultByReferenceBBox(originalCanvas, runningHubCanvas, referenceCanvas, { resultBBox = null } = {}) {
  ...
  const fitScale = Math.min(
    ...
    finalCanvas.height / Math.max(1, safeResultBBox.height),
  );
  const drawWidth = Math.round(safeResultBBox.width * fitScale);
  const drawHeight = Math.round(safeResultBBox.height * fitScale);
...
function clampBBox(bbox, width, height) {
  ...
  const maxX = Math.max(x, Math.min(width, bbox.x + bbox.width));
  const maxY = Math.max(y, Math.min(height, bbox.y + bbox.height));
  return {
    x,
    y,
...
async function requestRunningHubRemoveBgStatus(taskId, signal = null, proxyUrl = REMOVE_BG_PROXY_URL) {
  try {
...
function updateVideoPreviewBackground() {
  ...
  renderVideoPreviewFrame();
  if (videoPreviewItem) {
    setVideoPreviewStatus(type === "transparent"
      ? "透明背景预览中。下载需要真实透明 MOV；如果当前结果不是透明 MOV，会提示接入转码服务。"
      : "颜色背景预览已更新。下载将生成真实 MP4，不会使用 WebM 冒充。");
  }
}
...
async function downloadVideoPreviewOutput(item) {
  ...
  if (settings.type === "color" && videoPreviewState.exportedBlob && videoPreviewState.exportedSettingsKey === settingsKey) {
  ...
  try {
    if (settings.type === "transparent") {
      await downloadTransparentMovOutput(item);
      return;
    }
    ...
    if (!/mp4/i.test(mimeType || blob.type || "")) {
      throw new Error("当前浏览器无法直接导出 MP4，请接入转码服务后使用。");
    }
    ...
    setVideoPreviewStatus(`下载失败：${message}`);
  } finally {
...
async function downloadTransparentMovOutput(item) {
  if (isTransparentMovVideoItem(item)) {
    const outputName = makeTransparentMovOutputName(item.outputName || item.file?.name || "video");
    downloadBlob(item.blob, outputName);
    setVideoPreviewStatus("已下载 RunningHub 透明 MOV。");
    return;
  }
  throw new Error("当前浏览器无法直接导出透明 MOV，请接入转码服务后使用。");
}
...
async function renderVideoWithNativeMp4(item, backgroundSettings) {
  ...
  try {
    ...
    console.info("[video-background-export]", {
      backgroundType: backgroundSettings.type,
      selectedColor: backgroundSettings.color,
      videoWidth: width,
      videoHeight: height,
      mimeType,
      targetFormat: "mp4",
    ...
    const chunks = [];
    const exportScratch = createVideoRenderScratch();
    let rafId = 0;
    let drawing = true;
    ...
    const draw = () => {
      ...
      renderVideoFrameToCanvas({
        ...
        chromaOptions: DEFAULT_VIDEO_CHROMA_KEY,
        requiresLocalChromaKey: item.requiresLocalChromaKey !== false,
    ...
    if (blob.size < 1024) throw new Error("导出失败：生成的视频数据异常偏小。");
    return { blob, mimeType: recorder.mimeType || mimeType };
  } finally {
    sourceVideo.pause();
...
function createVideoRenderScratch() {
  return {
    sourceCanvas: null,
    foregroundCanvas: null,
  };
...
function renderVideoFrameToCanvas({
  ...
  drawVideoBackground(ctx, width, height, { type: backgroundType, color: backgroundColor });
  if (!requiresLocalChromaKey) {
    ctx.drawImage(video, 0, 0, width, height);
    return;
  }
  const sourceCanvas = getSizedCanvasFromScratch(scratch, "sourceCanvas", width, height);
  const foregroundCanvas = getSizedCanvasFromScratch(scratch, "foregroundCanvas", width, height);
  const sourceCtx = sourceCanvas.getContext("2d", { willReadFrequently: true });
  ...
  applyVideoChromaKeyToImageData(imageData, chromaOptions);
...
function getSizedCanvasFromScratch(scratch, key, width, height) {
  if (scratch === videoPreviewState) return getSizedReusableCanvas(key, width, height);
...
function applyVideoChromaKeyToImageData(imageData, options) {
  ...
  const dominanceThreshold = Math.max(30, tolerance * 0.45);
  for (let index = 0; index < data.length; index += 4) {
    const r = data[index];
    ...
    const redGap = g - r;
    const blueGap = g - b;
    const ratioGreen = g > 80 && g > r * 1.18 && g > b * 1.18 && redGap > 22 && blueGap > 22;
    const strongGreen = g > 80 && greenAdvantage > dominanceThreshold && redGap > dominanceThreshold * 0.7 && blueGap > dominanceThreshold * 0.7;
    const keyDistanceMatte = 1 - smoothstep(tolerance * 0.9, tolerance * 0.9 + softness * 1.35, distance);
    const dominanceMatte = smoothstep(dominanceThreshold * 0.72, dominanceThreshold + softness, Math.min(redGap, blueGap));
    const ratioMatte = ratioGreen ? smoothstep(18, 18 + softness, Math.min(redGap, blueGap)) : 0;
    const backgroundAmount = clampNumber(Math.max(keyDistanceMatte, dominanceMatte, ratioMatte, strongGreen ? 0.92 : 0), 0, 1);
    ...
    if (options.despill && alpha > 0 && greenAdvantage > 4) {
...
function getVideoBackgroundSettingsKey(settings) {
  return `${settings.type}:${settings.type === "color" ? settings.color : "transparent"}`;
}

function setVideoPreviewExport(blob, name, settingsKey) {
  clearVideoPreviewExport();
  videoPreviewState.exportedBlob = blob;
  videoPreviewState.exportedUrl = URL.createObjectURL(blob);
  videoPreviewState.exportedName = name;
  videoPreviewState.exportedSettingsKey = settingsKey;
}

function clearVideoPreviewExport() {
  if (videoPreviewState.exportedUrl) URL.revokeObjectURL(videoPreviewState.exportedUrl);
  videoPreviewState.exportedBlob = null;
  videoPreviewState.exportedUrl = "";
  videoPreviewState.exportedName = "";
  videoPreviewState.exportedSettingsKey = "";
}

function markVideoPreviewExportDirty() {
  if (!videoPreviewState.exportedBlob || videoPreviewState.exportBusy) return;
  const settingsKey = getVideoBackgroundSettingsKey(getVideoBackgroundSettings());
  if (settingsKey !== videoPreviewState.exportedSettingsKey) clearVideoPreviewExport();
}
...
function getNativeMp4RecorderMimeType() {
...
function isTransparentMovVideoItem(item) {
  const kind = String(item?.resultKind || "").toLowerCase();
  const mime = String(item?.resultMimeType || item?.blob?.type || "").toLowerCase();
  const name = String(item?.outputName || item?.file?.name || "").toLowerCase();
  return item?.requiresLocalChromaKey === false
    && (kind === "transparent-mov" || mime.includes("quicktime") || mime.includes("mov") || /\.mov$/i.test(name));
}

function makeTransparentMovOutputName(name) {
  const base = (name || "video").replace(/\.[^.]+$/, "");
  return `${base}-transparent.mov`;
}

function makeColorBackgroundMp4OutputName(name) {
  const base = (name || "video").replace(/\.[^.]+$/, "");
  return `${base}-background.mp4`;
}
