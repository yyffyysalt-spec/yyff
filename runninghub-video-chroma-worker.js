...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      return errorResponse(error.stage || "worker", error.message || "RunningHub 视频抠绿幕失败", error.detail || "", error.status || 500, error.raw);
    }
  },
};
...
function getConfig(env) {
  const config = {
    workflowId: env.RUNNINGHUB_VIDEO_CHROMA_WORKFLOW_ID || "",
    inputNodeId: env.RUNNINGHUB_VIDEO_CHROMA_INPUT_NODE_ID || "",
    inputFieldName: env.RUNNINGHUB_VIDEO_CHROMA_INPUT_FIELD_NAME || "",
    outputNodeId: env.RUNNINGHUB_VIDEO_CHROMA_OUTPUT_NODE_ID || "",
    outputFieldName: env.RUNNINGHUB_VIDEO_CHROMA_OUTPUT_FIELD_NAME || "",
    keyColorNodeId: env.RUNNINGHUB_VIDEO_CHROMA_KEY_COLOR_NODE_ID || "",
    keyColorFieldName: env.RUNNINGHUB_VIDEO_CHROMA_KEY_COLOR_FIELD_NAME || "",
    toleranceNodeId: env.RUNNINGHUB_VIDEO_CHROMA_TOLERANCE_NODE_ID || "",
    toleranceFieldName: env.RUNNINGHUB_VIDEO_CHROMA_TOLERANCE_FIELD_NAME || "",
    featherNodeId: env.RUNNINGHUB_VIDEO_CHROMA_FEATHER_NODE_ID || "",
    featherFieldName: env.RUNNINGHUB_VIDEO_CHROMA_FEATHER_FIELD_NAME || "",
    spillNodeId: env.RUNNINGHUB_VIDEO_CHROMA_SPILL_NODE_ID || "",
    spillFieldName: env.RUNNINGHUB_VIDEO_CHROMA_SPILL_FIELD_NAME || "",
  };
  ...
  if (missing.length) {
    throw stageError("config", "RunningHub 视频抠绿幕工作流未配置", `缺少 ${missing.join(" / ")} 等参数。`, 501, { missing });
  }
  return config;
}
...
function getMissingConfig(config) {
  const missing = [];
  if (!config.workflowId) missing.push("RUNNINGHUB_VIDEO_CHROMA_WORKFLOW_ID");
  if (!config.inputNodeId) missing.push("RUNNINGHUB_VIDEO_CHROMA_INPUT_NODE_ID");
  if (!config.inputFieldName) missing.push("RUNNINGHUB_VIDEO_CHROMA_INPUT_FIELD_NAME");
  if (!config.outputNodeId) missing.push("RUNNINGHUB_VIDEO_CHROMA_OUTPUT_NODE_ID");
  if (!config.outputFieldName) missing.push("RUNNINGHUB_VIDEO_CHROMA_OUTPUT_FIELD_NAME");
  return missing;
}
...
async function createVideoTask(payload, apiKey, config) {
  ...
  const nodeInfoList = [
    ...
    ...buildOptionalNodeInfo(config.keyColorNodeId, config.keyColorFieldName, payload.keyColor),
    ...buildOptionalNodeInfo(config.toleranceNodeId, config.toleranceFieldName, payload.tolerance),
    ...buildOptionalNodeInfo(config.featherNodeId, config.featherFieldName, payload.edgeFeather),
    ...buildOptionalNodeInfo(config.spillNodeId, config.spillFieldName, payload.spillSuppression),
  ];
  console.log(`[${WORKER_NAME}] create_task_payload_summary`, {
    workflowId: config.workflowId,
    nodeInfoList: nodeInfoList.map((node) => ({ nodeId: node.nodeId, fieldName: node.fieldName, fieldValue: node.fieldValue })),
  });
  ...
  if (!response.ok || !taskId) throw stageError("create_task", "RunningHub 视频抠绿幕任务创建失败", summarizeData(data), 502, data);
  console.log(`[${WORKER_NAME}] create_task_done`, { taskId });
  return jsonResponse({ ok: true, status: "pending", stage: "create_task_done", taskId, workflowId: String(config.workflowId), message: "视频抠绿幕任务已创建" });
}
...
async function checkVideoTask(taskId, apiKey, config) {
  ...
  if (isFailure(data)) throw stageError("runninghub_status", getMessage(data) || "RunningHub 视频抠绿幕任务失败", summarizeData(data), 502, data);
  return jsonResponse({ ok: true, status: "running", stage: "runninghub_status", taskId, message: "RunningHub 正在处理视频抠绿幕" });
}

function buildOptionalNodeInfo(nodeId, fieldName, fieldValue) {
  if (!nodeId || !fieldName || fieldValue === undefined || fieldValue === null || fieldValue === "") return [];
  return [{ nodeId: String(nodeId), fieldName, fieldValue: String(fieldValue) }];
}
