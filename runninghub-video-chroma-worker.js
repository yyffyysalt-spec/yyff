  ...
  async fetch(request, env) {
    ...
    try {
      return await handleVideoWorkflow(request, env, {
        ...
        notConfiguredMessage: "视频抠绿幕工作流未配置，请先补充 RunningHub workflowId 和节点参数。",
      });
    } catch (error) {
      return errorResponse(error.stage || "worker", error.message || "视频抠绿幕失败", error.detail || "", error.status || 500, error.raw);
    }
  },
};
...
async function createVideoTask(file, apiKey, config) {
  ...
  return jsonResponse({ ok: true, status: "pending", taskId, workflowId: String(config.workflowId), message: "视频抠绿幕任务已创建" });
}
...
function healthResponse(env) {
  const configReady = Boolean(env.RUNNINGHUB_VIDEO_CHROMA_WORKFLOW_ID && env.RUNNINGHUB_VIDEO_CHROMA_INPUT_NODE_ID && env.RUNNINGHUB_VIDEO_CHROMA_INPUT_FIELD_NAME);
