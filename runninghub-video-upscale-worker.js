...
const DEFAULT_APP_ID = "2033815457634983938";
const DEFAULT_CREATE_ENDPOINT = "https://www.runninghub.cn/openapi/v2/run/ai-app/2033815457634983938";
  ...
  async fetch(request, env) {
    ...
    try {
      ...
      return errorResponse(
        error.stage || "worker",
        error.message || "RunningHub 视频高清放大失败",
        error.detail || "",
        error.status || 500,
        error.raw,
...
function getHealthConfig(env) {
  return {
    mode: "ai-app",
    appId: env.RUNNINGHUB_VIDEO_UPSCALE_APP_ID || DEFAULT_APP_ID,
    createEndpoint: env.RUNNINGHUB_VIDEO_UPSCALE_CREATE_ENDPOINT || DEFAULT_CREATE_ENDPOINT,
    apiDemoEndpoint: env.RUNNINGHUB_VIDEO_UPSCALE_API_DEMO_ENDPOINT || `${RUNNINGHUB_BASE_URL}/api/webapp/apiCallDemo`,
  };
}
...
function ensureConfig(env, config) {
  ...
  if (missing.length) {
    throw stageError("config", "RunningHub 视频高清放大 AI App 未配置完整", `缺少 ${missing.join(" / ")}。`, 501, { missing }, config);
  }
}

function getMissingConfig(env, config) {
  const missing = [];
  if (!config.appId) missing.push("RUNNINGHUB_VIDEO_UPSCALE_APP_ID");
...
async function createVideoTask(payload, apiKey, config) {
  ...
  if (!response.ok || !taskId) {
    throw stageError("create_task", "RunningHub 视频高清放大任务创建失败", summarizeData(data), 502, data, config);
  }
  return jsonResponse({
    ok: true,
    status: "pending",
...
async function checkVideoTask(taskId, apiKey, config) {
  ...
  if (isFailure(data)) {
    throw stageError("runninghub_status", getMessage(data) || "RunningHub 视频高清放大任务失败", summarizeData(data), 502, data, config);
  }
  return jsonResponse({
    ok: true,
    status: "running",
...
async function buildNodeInfoListFromAiAppDemo(apiKey, config, uploaded) {
  ...
  if (!response.ok || !nodeInfoList.length) {
    throw stageError("ai_app_demo", "无法读取 RunningHub 视频高清放大 AI App 调用参数", summarizeData(data), 502, data, config);
  }

  const mediaNode = findMediaInputNode(nodeInfoList);
  if (!mediaNode) {
    throw stageError("ai_app_demo", "RunningHub 视频高清放大 AI App 没有找到视频上传节点", summarizeData({ nodeInfoList }), 502, data, config);
  }
